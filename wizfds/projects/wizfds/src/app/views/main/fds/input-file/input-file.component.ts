...
export class InputFileComponent implements OnInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.cm.setCursor(this.uiState.inputFile.line, this.uiState.inputFile.ch);
    this.cm.scrollTo(this.uiState.inputFile.left, this.uiState.inputFile.top);
  }

  ngOnDestroy() {
    // Save last curstor state before destroy
    this.uiState.inputFile.line = this.cm.getCursor().line;
    this.uiState.inputFile.ch = this.cm.getCursor().ch;
    this.uiState.inputFile.left = this.cm.getScrollInfo().left;
    this.uiState.inputFile.top = this.cm.getScrollInfo().top;

    // Save user defined section
    this.saveFile();
   ...
   */
  public numberAmpers(cm?: CodeMirror.Editor) {
