	...
	CodeMirror.defineMode("fds", function () {
		...
		function define(style, string) {
			...
			for (var i = 0; i < split.length; i++) {
				...
				if (style == 'geom') geom[split[i]] = style;
		...
		define('keyword', '&BNDF &CLIP &CSVF &CTRL &DEVC &DUMP &GEOM &HEAD &HOLE &HVAC &INIT &ISOF &MATL &MESH &MISC &MULT &OBST &PART &PRES &PROF &PROP &RADI &RAMP &REAC &SLCF &SPEC &SURF &TABL &TAIL &TIME &TRNX &VENT &ZONE');
		...
		define('geom', 'FYI FACES ID IJK MATL_ID MOVE_ID N_LAT N_LEVELS N_LONG SPHERE_ORIGIN SPHERE_RADIUS SPHERE_TYPE SURF_ID TEXTURE_MAPPING TEXTURE_ORIGIN TEXTURE_SCALE VERTS VOLUS XB ZVALS');
		define('head', 'FYI CHID TITLE');
		define('hole', 'FYI COLOR CTRL_ID DEVC_ID EVACUATION ID MESH_ID MULT_ID RGB TRANSPARENCY XB');
		...
		define('vent', 'FYI COLOR CTRL_ID DEVC_ID DYNAMIC_PRESSURE EVACUATION ID IOR L_EDDY L_EDDY_IJ MB MESH_ID MULT_ID N_EDDY OUTLINE PBX PRESSURE_RAMP RADIUS REYNOLDS_STRESS RGB SPREAD_RATE SURF_ID TEXTURE_ORIGIN TMP_EXTERIOR TMP_EXTERIOR_RAMP TRANSPARENCY UVW VEL_RMS XB XYZ');
			...
			token: function (stream, state) {
				...
				if (state.inAmper) {
					...
					if (stream.peek() == '/') { // Quote found on this line
						...
						else if (state.amper == '&GEOM') { stream.eatSpace(); if (stream.peek() == '(') return brackets(); else if (stream.peek() == '=') { stream.next(); return geom[cur]; } else return null; }
